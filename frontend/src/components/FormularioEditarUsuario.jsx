...
const FormularioEditarUsuario = ({ usuario, onClose, canEditRole = false }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (canEditRole) dataToSend.rol = form.rol;

      const response = await axios.put(
        "https://m2t-backend.onrender.com/api/auth/updateMe",
        dataToSend,
      ...
      if (setUser && response.data && response.data.user) {
        ...
        setUser({ ...usuario, ...form, ...(canEditRole ? {} : { rol: usuario.rol }) });
      }
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      {canEditRole ? (
        ...
        <input value={form.rol} disabled readOnly />
      )}
